...
export default function AssignmentManagementPage() {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      // Detect if current user is a teacher and collect their class references
      ...
      try {
        if (user?.uid) {
          ...
          if (!teacherSnap.empty) {
            const teacherData = teacherSnap.docs[0].data() as any;
            const classRefs: any[] = Array.isArray(teacherData.classes)
              ? teacherData.classes
                  .map((c: any) => (typeof c?.path === "string" ? doc(db, c.path) : c))
                  .filter(Boolean)
              : [];

            if (classRefs.length === 0) {
              console.log("Teacher has no assigned classes; no assignments to show");
              setAssignments([]);
              setLoading(false);
              return;
            }

            // Firestore 'in' supports max 10. Batch queries when needed.
            const chunks: any[][] = [];
            for (let i = 0; i < classRefs.length; i += 10) {
              chunks.push(classRefs.slice(i, i + 10));
            }

            const snapshots = await Promise.all(
              chunks.map((chunk) => getDocs(query(baseQuery, where("classId", "in", chunk))))
            );
            assignmentDocs = snapshots.flatMap((s) => s.docs);
          } else {
            // Not a teacher (likely admin) – fetch all for the school
            const allSnap = await getDocs(baseQuery);
            assignmentDocs = allSnap.docs;
          }
        } else {
          // No user – fallback to school-level fetch
          const allSnap = await getDocs(baseQuery);
          assignmentDocs = allSnap.docs;
        }
      } catch (e) {
        console.warn("Teacher filter failed, falling back to school-level fetch:", e);
        const allSnap = await getDocs(baseQuery);
        assignmentDocs = allSnap.docs;
      }

      console.log("Assignments found:", assignmentDocs.length);
      
      const assignmentPromises = assignmentDocs.map(async (assignmentDoc: any) => {
