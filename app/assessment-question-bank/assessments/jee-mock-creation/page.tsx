...
export default function CreateJeeMockPage() {
  ...
  const extractSchoolId = (schoolRef: any): string | null => {
    ...
    if (schoolRef.id) return schoolRef.id;
    return null;
  };
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // TODO: Implement JEE mock creation persist logic
    ...
    <div className="min-h-screen flex bg-gradient-to-br from-slate-50 via-blue-50/30 to-purple-50/20">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 lg:p-10">
        <div className="w-full max-w-2xl mb-8">
          <h1 className="text-3xl lg:text-4xl font-light text-slate-800 mb-2 tracking-tight">Create JEE MOCK</h1>
          <p className="text-slate-500 font-light">Design a JEE mock assessment</p>
        </div>
          ...
          <div className="p-8 lg:p-10">
            <form className="space-y-8" onSubmit={handleSubmit}>
              <div className="space-y-2">
                ...
                <input className={`w-full bg-slate-50/50 border-2 ${errors.testName ? 'border-red-300' : 'border-slate-200/50'} rounded-2xl px-4 py-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-300 focus:bg-white/80 transition-all duration-200 font-light text-lg`} placeholder="Enter test name" value={testName} onChange={e => setTestName(e.target.value)} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input type="datetime-local" className={`w-full bg-slate-50/50 border-2 ${errors.startTime ? 'border-red-300' : 'border-slate-200/50'} rounded-2xl px-4 py-4 text-slate-800 focus:outline-none focus:border-blue-300 focus:bg-white/80 transition-all duration-200 font-light text-lg`} value={startTime} onChange={e => setStartTime(e.target.value)} />
                  {errors.startTime && <p className="text-sm text-red-500">{errors.startTime}</p>}
                </div>
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-slate-700 mb-3">End Time</label>
                  <input type="datetime-local" className={`w-full bg-slate-50/50 border-2 ${errors.endTime ? 'border-red-300' : 'border-slate-200/50'} rounded-2xl px-4 py-4 text-slate-800 focus:outline-none focus:border-blue-300 focus:bg-white/80 transition-all duration-200 font-light text-lg`} value={endTime} onChange={e => setEndTime(e.target.value)} />
              ...
              <div className="space-y-2">
                ...
                <select className={`w-full bg-slate-50/50 border-2 ${errors.selectedClass ? 'border-red-300' : 'border-slate-200/50'} rounded-2xl px-4 py-4 text-slate-800 focus:outline-none focus:border-blue-300 focus:bg-white/80 transition-all duration-200 font-light text-lg`} value={selectedClass} onChange={e => setSelectedClass(e.target.value)} disabled={loadingClasses}>
              ...
              <div className="flex justify-end gap-4">
                ...
                <button type="submit" disabled={submitting} className="px-6 py-3 rounded-2xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50">{submitting ? 'Creating...' : 'Create JEE Mock'}</button>
              </div>
            </form>
          </div>
        </div>
